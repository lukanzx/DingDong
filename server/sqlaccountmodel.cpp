...
void SqlAccountModel::updateUserInfo(const int &userID, const QString &name, const int &gender, const QString &birth,
                                     const QString &area, const QString &education, const QString &signature) {
    setTable(accountTableName);
    setFilter(QString("ID = %1").arg(userID));
    if (!select()) {
        qDebug() << lastError();
...
QByteArray SqlAccountModel::messageList(const int &ID) {
    ...
    while (!jsonItem.empty()) jsonItem.removeLast();
    if (!query.exec(
            QString("SELECT GROUPMESSAGEINFO.GID AS GID, GROUPMESSAGEINFO.MID AS MID, GROUPMESSAGEINFO.DATETIME AS DATETIME, GROUPMESSAGEINFO.MESSAGE AS MESSAGE "
