...
void SqlFriendModel::sendMessage(const int &sendID, const int &receiveID, const int &type, const int &datetime,
                                 const QString &message) {
    QSqlQuery query;
    int id;
    if (!query.exec(QString("SELECT ID FROM FRIENDINFO WHERE "
                            "(USERID=%1 AND FRIENDID=%2) OR (USERID=%2 AND FRIENDID=%1)").arg(QString::number(sendID),
                                                                                              QString::number(
                                                                                                      receiveID)))) {
        qDebug() << "选择好友对编号发生错误";
        qDebug() << query.lastError();
    ...
    if (!query.exec(QString("INSERT INTO FRIENDMESSAGEINFO VALUES("
                            "%1, %2, %3, %4, '%5', %6)").arg(QString::number(id), QString::number(sendID),
                                                             QString::number(receiveID), QString::number(datetime),
