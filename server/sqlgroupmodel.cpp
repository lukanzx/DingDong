...
void SqlGroupModel::sendMessage(const int &gID, const int &mID, const int &type, const int &datetime,
                                const QString &message) {
    setTable(groupMessageTableName);
    QSqlQuery query;
    if (!query.exec(QString("INSERT INTO GROUPMESSAGEINFO VALUES("
                            "%1, %2, %3, '%4', %5)").arg(QString::number(gID), QString::number(mID),
                                                         QString::number(datetime), message, QString::number(type)))) {
        qDebug() << "添加群聊天信息发生错误";
        qDebug() << query.lastError();
    } else {
        qDebug() << "添加群聊天信息成功";
